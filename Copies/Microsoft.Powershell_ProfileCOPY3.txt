...
# =====================================================================
#  Merged helpers (zadd* / profile pull / ffp / alias)
# =====================================================================

#region ---------------- Zoxide Bulk Add ----------------
function zadd {
...
#region ---------------- Profile Pull (merged / renamed) ----------------
# Renamed from get_backup_psprofile_github -> get_psprofile_github
# Alias so calling by alias executes this command
Set-Alias -Name get_psprofile_github -Value get_psprofile_github -Option AllScope -Force
...
# Also provide the simpler no-backup version if preferred
function get-psprofile-raw {
    $url = "https://raw.githubusercontent.com/MichaelB11221/Docs-Easy/main/Microsoft.PowerShell_profile.ps1"
    try {
        Invoke-WebRequest -Uri $url -OutFile $PROFILE -ErrorAction Stop
        Write-Host "Profile pulled from GitHub successfully. Run 'reload-profile' to apply." -ForegroundColor Green
...
Set-Alias gpgh get_psprofile_github
#endregion

#region ---------------- FFP (File Find Pattern) ----------------
function ffp {
    param(
...
function Invoke-ExecAll {
    ...
    Write-Host "execall → Running all scripts and merging .reg files in:" -ForegroundColor Cyan
    Write-Host "   $currentDir" -ForegroundColor White

    # Define execution order (registry first is usually safest)
    $order = @('.reg', '.ps1', '.cmd', '.bat', '.vbs')
    
    $files = Get-ChildItem -File | 
             Where-Object { $_.Extension -in $order } |
             Sort-Object { $order.IndexOf($_.Extension) }, Name

    if (-not $files) {
        Write-Host "No supported files found (.reg, .ps1, .cmd, .bat, .vbs)" -ForegroundColor Yellow
        return
    }

    foreach ($file in $files) {
        Write-Host "`n→ $($file.Name)" -ForegroundColor Yellow -NoNewline
        Write-Host "  [$($file.Extension)]" -ForegroundColor DarkGray
        ...
        try {
            switch ($file.Extension.ToLower()) {
                '.reg' {
                    Write-Host "   Merging registry file..." -ForegroundColor Magenta
                    regedit.exe /s `"$($file.FullName)`"
                    Write-Host "   Registry merged" -ForegroundColor Green
                }
                '.ps1' {
                    & $file.FullName
                }
    ...
    Write-Host "`nexecall completed." -ForegroundColor Green
}

# Create the alias
Set-Alias -Name execall -Value Invoke-ExecAll -Force -Scope Global